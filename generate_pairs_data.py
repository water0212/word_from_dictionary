...
full_dict_file = os.path.join(current_dir,"TEST_DATA", 'FIX_HIT_cilin_utf8_no_empty_poly.txt')
...
def main():
        ...
        for split, target_n in splits.items():
            ...
            for cat in categories:
                ...
                if len(sampled_word_pairs) < target_n:
                    print(f"    ⚠️ 警告: {letter} 類的 {split} 在 {cat} 只能找到 {len(sampled_word_pairs)}/{target_n} 組。")


     # ==========================================
    # 【新增】步驟 4.5: 印出各類別實際抽取數量總結表
    # ==========================================
    print("\n" + "="*85)
    ...
    for letter in sorted(target_counts.keys()):
        for cat in categories:
            ...
            # 取得實際成功抽取的數量 (直接計算 output_data 裡的長度)
